...
public class DatabaseUtil {
    ...
    public static Note saveNote(String username, Note note) {
        try {
            if (note.getId() <= 0) { // New note
                return insertNote(username, note);
            } else { // Existing note
    ...
    private static Note insertNote(String username, Note note) throws SQLException {
    ...
    public static Note addAndReturnNote(String username, String content, String category) {
        String sql = "INSERT INTO notes (user_id, content, category) VALUES " +
                   "((SELECT id FROM users WHERE username = ?), ?, ?)";
        try (Connection conn = getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            try (ResultSet rs = stmt.getGeneratedKeys()) {
                if (rs.next()) {
                    return new Note(rs.getInt(1), content, category);
                }
            }
            return null;
        } catch (SQLException e) {
    ...
    // Change this method signature in DatabaseUtil.java
    ...
    public static List<Note> getUserNotes(String username) {
        ...
        String sql = "SELECT n.id, n.content, n.category FROM notes n " +
                    "JOIN users u ON n.user_id = u.id " +
                    "WHERE u.username = ? ORDER BY n.id DESC";
        try (Connection conn = getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, username);
            try (ResultSet rs = stmt.executeQuery()) {
...
class Note {
    ...
    public Note(String content, String category) {
        this(0, content, category); // ID 0 indicates new note
    }
